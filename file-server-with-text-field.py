...
"""
HTTP Upload/Download Server with Folder Navigation and Code Storage

A minimal HTTP server that supports file uploads, downloads, folder navigation,
and code storage for copying/pasting between computers.
"""
...
<head>
    ...
    <style>
        ...
        .upload-section, .files-section, .code-section {
            flex: 1;
            min-width: 300px;
...
<body>
    ...
    <div class="container">
        ...
        <div class="code-section">
            <h2>Code Storage</h2>
            <textarea id="codeInput" rows="10" placeholder="Paste or type code here"></textarea>
            <div class="button-group">
                <button id="saveCodeButton" aria-label="Save code as file">Save Code</button>
                <button id="loadCodeButton" aria-label="Load code from file">Load Code</button>
            </div>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            saveCodeButton.addEventListener('click', () => {
                ...
                if (!code.trim()) {
                    showStatusMessage('No code to save', 'error');
                    return;
                }
                const blob = new Blob([code], { type: 'text/plain' });
                const formData = new FormData();
                formData.append('path', currentPath);
                formData.append('files', blob, 'saved_code.txt');
                uploadFormData(formData);
            });
            
            loadCodeButton.addEventListener('click', () => {
                const filePath = currentPath ? `${currentPath}/saved_code.txt` : 'saved_code.txt';
                fetch(`/download?path=${encodeURIComponent(filePath)}`)
                    ...
                    .then(text => {
                        ...
                        showStatusMessage('Code loaded successfully', 'success');
                    })
                    .catch(error => {
                        showStatusMessage(`Error loading code: ${error.message}`, 'error');
                    });
            });
